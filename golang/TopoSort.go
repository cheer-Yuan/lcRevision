...
动态规划：dp[mouse][cat][turns], 0 <= mouse, cat < n
边界条件: dp[0][cat][turns] = 1 为老鼠的必胜状态，猫的必败状态, dp[cat][cat][turns] = 2 为老鼠的必败状态，猫的必胜状态, turns >= 2n(n - 1) 为双方的必和状态 (>=所有可能的局面数)

如果轮到 老鼠/猫 移动，则对于 老鼠/猫 从当前节点移动一次之后可能到达的每个节点，进行如下操作：
